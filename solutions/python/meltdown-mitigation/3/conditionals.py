...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    criticality_balanced = temperature < 800 and neutrons_emitted > 500 and product < 500000
    
    if criticality_balanced:
        return True
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    calculate = temperature * neutrons_produced_per_second
    low = threshold * 0.9
    # python supports supports chaining comparisons like math notation — x ≤ calculate ≤ y 
    # called 'Python's chained comparison syntax'
    normal = threshold * 0.9 <= calculate <= threshold * 1.1
   
    if calculate < low:
        return  'LOW'
    if normal:
        return 'NORMAL'
    if not normal or not low:
        return 'DANGER'